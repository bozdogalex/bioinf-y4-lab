...
def init_score_matrix_global(m: int, n: int, gap: int):
    ...
    # matrice (m+1) x (n+1) cu 0 inițial
    score = [[0] * (n + 1) for _ in range(m + 1)]

    # prima coloană (gap penalties)
    for i in range(m + 1):
        score[i][0] = i * gap

    # primul rând (gap penalties)
    for j in range(n + 1):
        score[0][j] = j * gap
...
def main():
    ...
    # AM limitat putin aici, imi aparea in terminal TERMINATED, probabil nu era suficient RAM..
