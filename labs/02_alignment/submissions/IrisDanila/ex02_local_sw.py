...
def init_score_matrix_local(m: int, n: int):
    ...
    """
    # Pentru SW, toată matricea începe cu 0 (inclusiv marginile)
...
def score_cell_local(score, i: int, j: int, a: str, b: str, match: int, mismatch: int, gap: int):
    """
    Calculați scorul unei celule (i, j) pentru SW.
    """
...
def main():
    ...
    print(f"{id1}  vs  {id2}")
    print(f"Lungimi originale: {len(s1)} vs {len(s2)}")
    print(f"Score: {sc}")
