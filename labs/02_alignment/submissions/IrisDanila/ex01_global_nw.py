...
def init_score_matrix_global(m: int, n: int, gap: int):
    ...
    """
    # Creăm o matrice (m+1) x (n+1) plină cu 0
    score = [[0 for _ in range(n + 1)] for _ in range(m + 1)]
    
    # Prima coloană: penalități cumulative pentru gap-uri pe seq1
    for i in range(m + 1):
        score[i][0] = i * gap
    
    # Prima linie: penalități cumulative pentru gap-uri pe seq2
    for j in range(n + 1):
        score[0][j] = j * gap
...
def score_cell_global(score, i: int, j: int, a: str, b: str, match: int, mismatch: int, gap: int):
    """
    Calculați scorul unei celule (i, j) pentru NW.
    """
    # Scor diagonal: match sau mismatch
    if a == b:
        diagonal = score[i-1][j-1] + match
    else:
        diagonal = score[i-1][j-1] + mismatch
    
    # Scor de sus: gap în seq2
    up = score[i-1][j] + gap
    
    # Scor de la stânga: gap în seq1
...
def needleman_wunsch(seq1: str, seq2: str, match=1, mismatch=-1, gap=-2):
    ...
    # Completăm cu gap-uri la margine
    while i > 0:
        align1 = seq1[i - 1] + align1
        align2 = "-" + align2
        i -= 1
...
def main():
    ...
    # Truncate sequences for demo (optional - remove for full alignment)
    max_len = 500
    if len(s1) > max_len or len(s2) > max_len:
        print(f"[info] Secvențele sunt lungi. Trunchez la {max_len} pentru demo.")
    ...
    print(f"{id1}  vs  {id2}")
    print(f"Lungimi: {len(s1)} vs {len(s2)}")
    print(f"Score: {sc}")
