...
def init_score_matrix_local(m: int, n: int):
    ...
    """
    # Matricea (m+1) x (n+1) umpluta cu 0.
...
def score_cell_local(score, i: int, j: int, a: str, b: str, match: int, mismatch: int, gap: int):
    ...
    """
    # Stabilim recompensa/penalizarea
    match_or_mismatch = match if a == b else mismatch

    # 1. Scorul venit de pe diagonala (Match/Mismatch)
    diagonal = score[i - 1][j - 1] + match_or_mismatch

    # 2. Scorul venit de sus (Gap în secvenaa 2)
    up = score[i - 1][j] + gap

    # 3. Scorul venit de la stanga (Gap în secventa 1)
    left = score[i][j - 1] + gap

    # 4. SW: Rezultatul este maximul dintre 0 si cele trei cai posibile.
