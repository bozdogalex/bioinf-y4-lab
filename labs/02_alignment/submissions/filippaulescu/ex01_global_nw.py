...
def init_score_matrix_global(m: int, n: int, gap: int):
    ...
    # 2. Initializam prima coloana cu penalizarea de gap: [i * gap]
    for i in range(m + 1):
        score[i][0] = i * gap

    # 3. Initializam prima linie cu penalizarea de gap: [j * gap]
    for j in range(n + 1):
        score[0][j] = j * gap
...
def score_cell_global(score, i: int, j: int, a: str, b: str, match: int, mismatch: int, gap: int):
    ...
    """
    # 1. Stabilim penalizarea/recompensa pentru alinierea curentă (a vs b).
    #    a si b sunt caracterele (nucleotidele) curente.
    match_or_mismatch = match if a == b else mismatch

    # 2. Calculam scorul venit de pe diagonala (alegere: Match/Mismatch)
    #    Score[i-1][j-1] + (recompensa sau penalizarea)
    diagonal = score[i - 1][j - 1] + match_or_mismatch

    # 3. Calculam scorul venit de sus (alegere: Gap în secventa 2)
    #    Score[i-1][j] + penalizarea de gap
    up = score[i - 1][j] + gap

    # 4. Calculam scorul venit de la stanga (alegere: Gap în secventa 1)
    #    Score[i][j-1] + penalizarea de gap
    left = score[i][j - 1] + gap

    # 5. Returnam scorul maxim
