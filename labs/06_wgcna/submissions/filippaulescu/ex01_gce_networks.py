...
INPUT_CSV = Path("data/work/filippaulescu/lab06/expression_matrix1.csv")
OUTPUT_DIR = Path("labs/06_wgcna/submissions/filippaulescu")
OUTPUT_CSV = OUTPUT_DIR / "modules_filippaulescu.csv"
...
def read_expression_matrix(path: Path) -> pd.DataFrame:
    if not path.exists():
        raise FileNotFoundError(
            f"Nu am găsit {path}. E nevoie de matricea de expresie la această locație."
        )
