...
# --------------------------
INPUT_CSV = Path("data/work/RAZVAN24RR/lab06/expression_matrix.csv")
OUTPUT_DIR = Path("labs/06_wgcna/submissions/RAZVAN24RR")
OUTPUT_CSV = OUTPUT_DIR / "modules_RAZVAN24RR.csv"
...
def read_expression_matrix(path: Path) -> pd.DataFrame:
    ...
    # Citim doar primele 3000 de rânduri pentru a evita problemele de memorie (Codespaces limitation)
    df = pd.read_csv(path, index_col=0, nrows=3000)
    
    if df.empty:
        raise ValueError("Matricea de expresie este goală.")
    
    # Păstrăm doar datele numerice
...
def log_and_filter(df: pd.DataFrame,
    ...
    """
    # Optimizare: calculăm varianța înainte de log pentru a reduce memoria necesară
    variances = df.var(axis=1)
    
    # Păstrăm top 500 gene cele mai variabile
    top_genes = variances.sort_values(ascending=False).head(500).index
    df_subset = df.loc[top_genes]
    
    df_log = np.log2(df_subset + 1)
    return df_log
