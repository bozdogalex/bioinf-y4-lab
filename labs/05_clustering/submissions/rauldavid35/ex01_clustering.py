...
if __name__ == "__main__":
    ...
    output_dir = Path("labs/05_clustering/submissions/rauldavid35")
    ...
    dendrogram(Z)
    plt.title("Hierarchical Clustering Dendrogram (Breast Cancer WDBC)")
    plt.xlabel("Samples")
    plt.ylabel("Distance")
    plt.tight_layout()
    plt.savefig(output_dir / "hierarchical_rauldavid35.png")
    plt.close()
    ...
    plt.scatter(X_pca[:, 0], X_pca[:, 1], c=kmeans_labels, s=30, alpha=0.7)
    plt.title("K-means Clustering (K=2) pe date PCA")
    plt.xlabel("PCA 1")
    plt.ylabel("PCA 2")
    plt.tight_layout()
    plt.savefig(output_dir / "kmeans_rauldavid35.png")
    plt.close()
    ...
    # folosim același X_pca pentru vizualizare
    plt.figure(figsize=(8, 6))
    plt.scatter(X_pca[:, 0], X_pca[:, 1], c=dbscan_labels, s=30, alpha=0.7)
    plt.title("DBSCAN Clustering pe date PCA")
    plt.xlabel("PCA 1")
    plt.ylabel("PCA 2")
    plt.tight_layout()
    plt.savefig(output_dir / "dbscan_rauldavid35.png")
    plt.close()
    ...
    df[["Diagnosis", "KMeans_Cluster", "DBSCAN_Cluster"]].to_csv(
        output_dir / "clusters_rauldavid35.csv", index=False
    )
