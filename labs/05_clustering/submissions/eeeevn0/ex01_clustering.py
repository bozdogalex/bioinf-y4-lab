...
if __name__ == "__main__":
    ...
    plt.savefig(output_dir / f"hierarchical_{handle}.png", dpi=300)
    plt.close()
    ...
    plt.savefig(output_dir / f"kmeans_{handle}.png", dpi=300)
    plt.close()
    ...
    plt.ylabel("PCA 2")
    plt.colorbar(label="Cluster")
    plt.savefig(output_dir / f"dbscan_{handle}.png", dpi=300)
    plt.close()
    ...
    df_output = df[["Diagnosis", "KMeans_Cluster", "DBSCAN_Cluster"]]
    df_output.to_csv(output_dir / f"clusters_{handle}.csv", index=False)
