...
if __name__ == "__main__":
    ...
    output_dir = Path("labs/05_clustering/submissions/banmepls")
    ...
    plt.savefig(output_dir / "hierarchical_banmepls.png")
    plt.close()
    ...
    plt.savefig(output_dir / "kmeans_banmepls.png")
    plt.close()
    ...
    plt.ylabel("PCA 2")
    plt.colorbar(label="Cluster")
    plt.savefig(output_dir / "dbscan_banmepls.png")
    plt.close()

    # Salvare rezultate
    # salvați un CSV cu coloanele ["Diagnosis", "KMeans_Cluster", "DBSCAN_Cluster"]
    # în clusters_<handle>.csv
    df[["Diagnosis", "KMeans_Cluster", "DBSCAN_Cluster"]].to_csv(output_dir / "clusters_banmepls.csv", index=False)
