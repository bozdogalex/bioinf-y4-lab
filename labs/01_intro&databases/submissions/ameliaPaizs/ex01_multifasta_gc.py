...
def download_fasta(email: str, out_path: Path, query: str = None,
    ...
    """
    """Descărcare FASTA de la NCBI folosind Entrez."""
    Entrez.email = email
    if api_key:
    ...
    # --- Caz 1: avem accession ---
    if accession:
        ids = [accession]

    # --- Caz 2: avem query ---
    elif query:
        print(f"[info] Caut {query} în baza {db} ...")
        handle = Entrez.esearch(db=db, term=query, retmax=retmax)
        results = Entrez.read(handle)
        ids = results["IdList"]
        handle.close()
        if not ids:
            print("[warn] Niciun ID găsit pentru query-ul dat.")
            return 0
    else:
        raise ValueError("Trebuie să specificați --accession sau --query")

    # --- Descarcă FASTA pentru aceste ID-uri ---
    print(f"[info] Descarc {len(ids)} înregistrări din NCBI...")
    handle = Entrez.efetch(db=db, id=",".join(ids), rettype="fasta", retmode="text")
    fasta_data = handle.read()
    handle.close()

    out_path.write_text(fasta_data)
    print(f"[ok] Fișier salvat: {out_path}")
...
def main():
    ...
    # --- Calcul GC ---
    print("[info] Calculez fracția GC...")
    records = SeqIO.parse(str(out_path), "fasta")
    for rec in records:
